  ...
  <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p class="text-xs text-slate-400 mt-1">管理供应商基础信息</p>
      </div>
      <div class="flex items-center gap-3">
        <el-input v-model="keyword" placeholder="搜索供应商名称或编码" clearable class="w-56" @change="fetchData">
      ...
      <el-table :data="list" border stripe v-loading="loading" class="page-table">
        ...
        <el-table-column label="状态" width="100" align="center">
          <template #default="{ row }">
            ...
            <el-tag v-else type="danger" effect="light" size="small">停用</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="visible" :title="editing.id ? '编辑供应商' : '新增供应商'" width="560px" class="custom-dialog">
      <el-form :model="form" label-width="80px" label-position="right">
        <el-row :gutter="20">
          <el-col :span="12"><el-form-item label="编码"><el-input v-model="form.code" placeholder="请输入供应商编码" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="名称"><el-input v-model="form.name" placeholder="请输入供应商名称" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12"><el-form-item label="联系人"><el-input v-model="form.contact" placeholder="请输入联系人" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="电话"><el-input v-model="form.phone" placeholder="请输入电话" /></el-form-item></el-col>
        </el-row>
...
async function fetchData() {
  ...
  try {
    ...
    if (r.code === 200) {
      ...
      console.log('供应商数据加载成功:', list.value.length, '条, 总计:', total.value)
    } else {
      console.warn('供应商API返回非200:', r)
    }
  } catch (e) {
