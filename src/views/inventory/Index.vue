    ...
    <div class="px-6 py-5 border-b border-slate-100">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-1">库存查询、出入库操作和流水记录</p>
        </div>
      </div>
    </div>
    ...
    <div class="p-6">
      <el-tabs v-model="activeTab" class="custom-tabs" @tab-change="onTabChange">
        <el-tab-pane label="库存查询" name="stock">
          <div class="mb-4">
            <el-select v-model="filterProductId" placeholder="筛选产品" filterable clearable class="w-64" @change="fetchStock">
        ...
        <el-tab-pane label="库存流水" name="log">
          <el-table :data="transactions" border stripe class="page-table">
            ...
            <el-table-column prop="quantity" label="数量" width="120" align="right">
              <template #default="{ row }">
                <span :class="row.type === 'in' ? 'text-emerald-600' : 'text-orange-500'" class="font-medium">
                  {{ row.type === 'in' ? '+' : '-' }}{{ row.quantity }}
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="beforeQty" label="变更前" width="120" align="right" />
            <el-table-column prop="afterQty" label="变更后" width="120" align="right">
...
const activeTab = ref('stock'), stocks = ref([]), transactions = ref([]), filterProductId = ref()
...
async function fetchStock() { const r = filterProductId.value ? await api.get('/inventory', { params: { productId: filterProductId.value } }) : await api.get('/inventory'); if (r.code === 200) stocks.value = r.data }
async function fetchLog() {
  try {
    const r = await api.get('/inventory/transactions', { params: { pageSize: 99 } })
    if (r.code === 200) {
      // 兼容后端不同返回格式: data.list / data.records / data (数组)
      const data = r.data
      if (Array.isArray(data)) transactions.value = data
      else if (data?.list) transactions.value = data.list
      else if (data?.records) transactions.value = data.records
