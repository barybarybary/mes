<template>
  <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    ...
    <el-dialog v-model="visible" :title="editing.id ? '编辑客户' : '新增客户'" width="560px" class="custom-dialog">
      <el-form :model="form" label-width="80px" label-position="right">
        <el-row :gutter="20">
          <el-col :span="12"><el-form-item label="客户编码"><el-input v-model="form.code" placeholder="请输入客户编码" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="客户名称"><el-input v-model="form.name" placeholder="请输入客户名称" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="20">
...
async function fetchData() {
  ...
  try {
    ...
    const r = await api.get('/base/customer', { params })
    if (r.code === 200) {
      const data = r.data
      ...
      console.log('客户数据加载成功:', list.value.length, '条, 总计:', total.value)
    } else {
      console.warn('客户API返回非200:', r)
    }
  } catch (e) {
...
function openDialog(row) {
  editing.value = row || {}
  Object.assign(form, { code: row?.code || '', name: row?.name || '', contact: row?.contact || '', phone: row?.phone || '', email: row?.email || '', address: row?.address || '' })
  visible.value = true
}

async function save() {
  const d = { ...form }
  if (editing.value.id) { d.id = editing.value.id; await api.put('/base/customer', d) }
  else await api.post('/base/customer', d)
  ElMessage.success('保存成功'); visible.value = false; fetchData()
}

async function del(id) {
  await ElMessageBox.confirm('确定要删除该客户吗？', '提示', { type: 'warning', confirmButtonText: '确定', cancelButtonText: '取消' })
  await api.delete(`/base/customer/${id}`); ElMessage.success('删除成功'); fetchData()
}
