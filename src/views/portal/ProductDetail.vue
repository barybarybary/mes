<template>
  <div class="min-h-screen bg-slate-50">
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      <div v-else-if="!product" class="text-center py-10 text-slate-400">产品不存在</div>
      <div v-else class="bg-white rounded-xl shadow-sm border p-6 md:p-8">
        <div class="grid md:grid-cols-2 gap-8">
          <!-- 产品图 -->
          <div class="h-60 md:h-80 bg-gradient-to-br from-slate-100 to-slate-200 rounded-xl flex items-center justify-center text-6xl text-slate-400">📦</div>
          <!-- 信息 -->
          <div>
            <p class="text-xs text-slate-400 mb-1">{{ product.code }}</p>
            <h1 class="text-2xl font-bold text-slate-800 mb-2">{{ product.name }}</h1>
            <p class="text-slate-500 mb-1">规格：{{ product.spec || '-' }}</p>
            <p class="text-slate-500 mb-1">单位：{{ product.unit }}</p>
            <p class="text-slate-500 mb-4">库存：{{ product.stockQuantity > 0 ? `${product.stockQuantity} ${product.unit || 'pcs'}` : '暂时缺货' }}</p>
            ...
            <p v-if="product.remark" class="mt-6 text-sm text-slate-500 leading-relaxed border-t pt-4">{{ product.remark }}</p>
          </div>
        </div>
      </div>
    </div>
