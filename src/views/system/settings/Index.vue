  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      <div class="px-5 sm:px-6 py-4 border-b border-slate-50">
        ...
        <p class="text-xs text-slate-400 mt-1">选择系统的主色调</p>
      </div>
      <div class="p-5 sm:p-6">
        <div class="flex flex-wrap gap-4">
          <button
            ...
            :class="currentTheme === theme.name ? 'border-current shadow-md' : 'border-slate-200 hover:border-slate-300'"
        ...
        <div class="mt-5 flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-slate-50 to-slate-100 border border-slate-100">
          ...
          <div>
            <p class="text-sm font-medium text-slate-700">{{ currentLabel }}</p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      <div class="px-5 sm:px-6 py-4 border-b border-slate-50">
        ...
        <p class="text-xs text-slate-400 mt-1">切换亮色/暗色界面风格</p>
      </div>
      <div class="p-5 sm:p-6">
        <div class="flex items-center justify-between py-2">
          <div>
            <span class="text-sm font-medium text-slate-700">启用深色模式</span>
            <p class="text-xs text-slate-400 mt-1">降低屏幕亮度，适合夜间使用</p>
          </div>
          <el-switch
            :model-value="isDark"
            @change="toggleDark"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      <div class="px-5 sm:px-6 py-4 border-b border-slate-50">
        ...
        <p class="text-xs text-slate-400 mt-1">设置面包屑导航的展示方式</p>
      </div>
      <div class="p-5 sm:p-6">
        <el-radio-group :model-value="breadcrumbMode" @change="setBreadcrumbMode" class="flex flex-col gap-4">
          <div
            ...
            :class="breadcrumbMode === 'simple' ? 'border-sky-400 bg-sky-50/50' : 'border-slate-100 hover:border-slate-200 bg-white'"
            @click="setBreadcrumbMode('simple')"
          >
            <el-radio value="simple" class="!mr-0 mt-0.5" />
            <div>
              <p class="text-sm font-medium text-slate-700">简洁模式</p>
              <p class="text-xs text-slate-400 mt-0.5">仅显示「首页 / 当前页面」</p>
            </div>
          </div>
          <div
            class="flex items-start gap-3 p-4 rounded-xl border-2 transition-all duration-200 cursor-pointer"
            :class="breadcrumbMode === 'hierarchical' ? 'border-sky-400 bg-sky-50/50' : 'border-slate-100 hover:border-slate-200 bg-white'"
            @click="setBreadcrumbMode('hierarchical')"
          >
            <el-radio value="hierarchical" class="!mr-0 mt-0.5" />
            <div>
              <p class="text-sm font-medium text-slate-700">层级模式</p>
              <p class="text-xs text-slate-400 mt-0.5">显示完整菜单路径，如「首页 / 系统管理 / 用户管理」</p>
            </div>
          </div>
        </el-radio-group>
      </div>
